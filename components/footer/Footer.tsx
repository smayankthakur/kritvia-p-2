...
export function Footer({ settings }: FooterProps) {
  ...
  // Use settings footer navigation or fallback to hardcoded navigation
  const footerLinks = settings?.footerNavigation?.reduce((acc, item) => {
    // Group by first word or create a default group
    const groupName = item.title.split(' ')[0] || 'More'
    if (!acc[groupName]) {
      acc[groupName] = []
    }
    acc[groupName].push({
        ...
        <div className="py-6 border-t border-white/10">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            <p className="text-sm text-slate-500">
              © {currentYear} {settings?.title || 'Kritvia'}. All rights reserved.
            </p>
